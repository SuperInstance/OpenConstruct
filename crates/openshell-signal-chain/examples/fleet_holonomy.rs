...
use openshell_signal_chain::{Dial, HolonomyChain, HolonomyRoom};
...
fn main() {
    ...
    for room in ["scouting", "transit", "dockside", "storm"] {
        let _rooms: Vec<_> = (0..)
            .zip(chain.rigid_rooms().iter())
            .filter(|(_, n)| *n == room)
            .collect();
        if let Some(_r) = chain.rigid_rooms().iter().position(|n| *n == room) {
            println!("{}: rigid ✓", room);
        } else {
