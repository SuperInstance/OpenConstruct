...
use openshell_signal_chain::{
    BettiResult, Dial, HolonomyRoom, Room, SignalChain, SplineConstraint,
};

fn bench_dial(c: &mut Criterion) {
    let mut g = c.benchmark_group("Dial");
    g.bench_function("new_hard", |b| {
        b.iter(|| {
            let d = Dial::hard();
            black_box(d.position);
        })
    });
    g.bench_function("new_soft", |b| {
        b.iter(|| {
            let d = Dial::soft();
            black_box(d.position);
        })
    ...
    for pos in [0.0, 0.25, 0.5, 0.75, 1.0] {
        g.bench_with_input(BenchmarkId::from_parameter(pos), &pos, |b, &p| {
            b.iter(|| {
                let d = Dial::new(p);
                black_box(d.snap_weight());
                black_box(d.inference_threshold());
            });
        });
    }
    g.finish();
...
fn bench_spline(c: &mut Criterion) {
    ...
    for val in [10_i32, 35, 60, 90, 120] {
        g.bench_with_input(BenchmarkId::from_parameter(val), &val, |b, &v| {
            b.iter(|| {
                let d = wave.curvature_distance(v);
                black_box(d);
            });
        });
    }
    g.bench_function("maritime_preset", |b| {
        b.iter(|| {
...
fn bench_room(c: &mut Criterion) {
    ...
    let room = {
        ...
        for i in 0..100 {
            r.add_snap(serde_json::json!({"i":i}), 1.0);
        }
        for i in 0..50 {
            r.add_inference(serde_json::json!({"i":i}), 0.7);
        }
        r
    };
...
fn bench_holonomy(c: &mut Criterion) {
    ...
    for &(v, e) in &cases {
        g.bench_with_input(
            BenchmarkId::new("betti", format!("V{}E{}", v, e)),
            &(v, e),
            |b, &(v, e)| {
                b.iter(|| {
...
fn bench_chain(c: &mut Criterion) {
    ...
    for d in [0.0, 0.5, 1.0] {
        g.bench_with_input(BenchmarkId::from_parameter(d), &d, |b, &p| {
            b.iter(|| {
                black_box(chain.query_all(Dial::new(p)));
            });
        });
    }
    g.bench_function("traverse", |b| {
        b.iter(|| {
            black_box(chain.traverse(&["nav", "sonar", "weather", "analysis"]));
        })
    });
    g.finish();
}
...
fn bench_tput(c: &mut Criterion) {
    ...
    g.bench_function("add_snap_10k", |b| {
        b.iter(|| {
            let mut r = Room::new("t");
            for _ in 0..10_000 {
                r.add_snap(serde_json::json!({"x":1}), 1.0);
            }
            black_box(r)
        })
    });
    g.bench_function("add_inference_10k", |b| {
        b.iter(|| {
            let mut r = Room::new("t");
            for _ in 0..10_000 {
                r.add_inference(serde_json::json!({"x":1}), 0.75);
            }
            black_box(r)
        })
    });
