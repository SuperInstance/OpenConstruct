...
| **Rooms grow masks + residency heat** | Onboarding is growth, not configuration ([RFC 0004](rfc/0004-the-room-grows-a-mask/)). Every room derives a locked mask from its seed at creation — an ontological lattice of what its world contains — records hash-chained `walks/2` telemetry, and is read through residency heat (warm/cooling/cold) that modulates gravity and feeds Ensign attention priors. Keepers onboard from the room's growth record. Operator's guide: [docs/RESIDENCY.md](docs/RESIDENCY.md). |
